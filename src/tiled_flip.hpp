#ifndef ALG_TILED_FLIP
#define ALG_TILED_FLIP
                ...
                for (unsigned i = 0u; i < m; i++){
                    for(unsigned j = 0u; j < p; j++){
                        for(unsigned k = 0u; k < n; k++){
                            dest[i][j + d_off] += A[i][k + a_off] * B[j][k + b_off];
                        }
                    }
                }
            }
                ...
                for(unsigned i = 0u; i < num_tiles; i++){
                    ...
                    for(unsigned j = 0u; j < num_tiles; j++){
                        ...
                        for(unsigned k = 0u; k < num_tiles; k ++){
                            ...
                            ord_mult(A + a_row_idx, B + b_col_idx, tile_m, tile_n, tile_p, a_off, a_off, d_off, dest + a_row_idx);
                            a_off += option;
                        }
                        b_off += option;
                        d_off += option;
